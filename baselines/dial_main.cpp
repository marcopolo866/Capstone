...
InputData parse_input(const std::string& path) {
    ...
    InputData data;
    data.vertex_count = static_cast<int>(labels.size());
    ...
    data.rev_adj.assign(data.vertex_count, {});
    for (int u = 0; u < data.vertex_count; ++u) {
        for (const Edge& e : data.adj[u]) {
            if (e.to >= 0 && e.to < data.vertex_count) {
...
CxxGraphView build_cxxgraph(const InputData& input) {
    CxxGraphView view;
    view.nodes.reserve(static_cast<std::size_t>(input.vertex_count));
    ...
    for (int u = 0; u < input.vertex_count; ++u) {
        for (const Edge& e : input.adj[u]) {
            if (e.to < 0 || e.to >= input.vertex_count) {
                continue;
            }
...
std::optional<std::vector<long long>> try_run_cxxgraph_dial(const InputData& input, const CxxGraphView& graph_view, int start) {
    if (input.has_negative_weight) {
        return std::nullopt;
    }

    const long long bucket_count =
        static_cast<long long>(std::max(1, input.max_weight)) * static_cast<long long>(input.vertex_count);
    ...
    std::vector<long long> dist(static_cast<std::size_t>(input.vertex_count), INF_DIST);
    for (int i = 0; i < input.vertex_count; ++i) {
...
int main(int argc, char* argv[]) {
    ...
    try {
        InputData in = parse_input(argv[1]);
        CxxGraphView graph_view = build_cxxgraph(in);

        std::vector<long long> dist;
        std::vector<int> path;

        const auto dial_dist = try_run_cxxgraph_dial(in, graph_view, in.start);
        if (dial_dist) {
            dist = *dial_dist;
            path = reconstruct_path_from_dist(in.start, in.target, dist, in.rev_adj);
        } else {
            const ShortestPathResult fallback = run_dijkstra_fallback(in.adj, in.start);
            dist = fallback.dist;
            path = reconstruct_path_from_parent(in.start, in.target, fallback.parent);
        }

        if (in.target < 0 || in.target >= static_cast<int>(dist.size()) || dist[in.target] >= INF_DIST || path.empty()) {
            std::cout << "INF; (no path)\n";
            return 0;
        }

        std::cout << dist[in.target] << ';';
        for (int node : path) {
