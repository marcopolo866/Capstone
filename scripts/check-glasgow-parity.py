...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Check Glasgow baseline/ChatGPT/Gemini parity on a deterministic labeled LAD pair."
    )
    parser.add_argument(
        "--baseline-binary",
    ...
    if chatgpt_count != baseline_count:
        failed.append(
            f"ChatGPT mismatch: baseline={baseline_count}, chatgpt={chatgpt_count}"
        )
    if gemini_count != baseline_count:
        failed.append(
            f"Gemini mismatch: baseline={baseline_count}, gemini={gemini_count}"
        )
    ...
    print(f"Counts: baseline={baseline_count}, chatgpt={chatgpt_count}, gemini={gemini_count}")
