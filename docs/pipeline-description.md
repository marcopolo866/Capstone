...
In addition to the browser-triggered workflow path, the repo now includes a manifest-driven headless benchmark pipeline (`scripts/benchmark-runner.py`) plus dedicated GitHub Actions workflows for fully validated desktop packaging, smoke packaging, sanitizer validation, and direct CLI benchmark execution.
